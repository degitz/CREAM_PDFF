    ...
    for ka = 1:num_acqs
        for kc = 1:C
            for kn = 1:N
                for km = 1:N
                    if norm(r2starmap(:))==0
                        d1 = d1 - 1i*conj(images(:,:,kn,kc,ka)).*images(:,:,km,kc,ka)*Gamma(kn,km).*exp(1i*2*pi*fm*(t(kn)-t(km)))*2*pi*(t(kn)-t(km));
                    else
                        d1 = d1 - 1i*conj(images(:,:,kn,kc,ka)).*images(:,:,km,kc,ka).*curGamma(:,:,kn,km,ka).*exp(1i*2*pi*fm*(t(kn)-t(km)))*2*pi*(t(kn)-t(km));
                    end
                end
            end
        end
